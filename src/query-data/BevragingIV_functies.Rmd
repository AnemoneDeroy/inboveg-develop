...
- or define a part of the name of the survey by contains <- part_name
- or define the owner of the surveys you are looking for by onwner <- name_owner
...
survey_info <- function(Survey, string, owner) {
  dbGetQuery(con, glue_sql(
    ...
    WHERE Name like {survey} 
    OR string = filter(ivS, contains{string})
    OR owner like {owner})
    )
 }
