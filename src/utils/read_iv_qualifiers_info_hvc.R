...
inboveg_qualifiers_HVC <- function(connection,
                                        survey_name,
                                        multiple = FALSE) {
...
qualifiers_heischraal2012 <- inboveg_qualifiers_HVC(connection, survey_name = "MILKLIM_Heischraal2012")
qualifiers_milkim <- inboveg_qualifiers_HVC(connection, survey_name = "%MILKLIM%")
qualifiers_severalsurveys <- inboveg_qualifiers_HVC(connection, survey_name = c("MILKLIM_Heischraal2012", "NICHE Vlaanderen"), multiple = TRUE)
allqualifiers <- inboveg_qualifiers_HVC(connection)
...
debugonce(dbSendQuery)
dbSendQuery(conn = connection,
"SELECT * FROM (SELECT ivS.Name
        , ivR.RecordingGivid
        , ivR.UserReference
        , ivR.Observer
        , ivRLQ.QualifierType
        ...
        WHERE ivRLQ.ParentID Is Null
        AND ivS.Name LIKE 'MILKLIM_Heischraal2012'
        ORDER BY ivR.UserReference, ivRLQ.QualifierType, ivRLQ.QualifierCode OFFSET 0 ROWS)
      'zzz18'
       WHERE (0=1)")
